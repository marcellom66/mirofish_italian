          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Quick chat version of the report generation agent, with access to 4 specialized tools and full MiroFish memory</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Aligns real-world seed data with simulated environment state, combining Global/Local Memory mechanisms for cross-temporal deep attribution analysis</div>
                  </div>
                </div>
                ...
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Autonomous interviews that can conduct multi-round parallel conversations with individuals in the simulated world, collecting unstructured opinion data and psychological states</div>
                  </div>
                </div>
              </div>
            </div>
...
<script setup>
/**
 * Step5Interaction.vue — Deep Interaction panel (Step 5 of the pipeline)
 *
 * Provides post-simulation interaction capabilities:
 *   - Chat with the Report Agent (AI-powered Q&A about the simulation)
 *   - Chat with individual simulated agents (interview mode)
 *   - Multi-agent survey (send a question to multiple agents at once)
 *   - Report outline viewer with collapsible sections
 *   - Agent profile browser with tool descriptions
 *
 * Features:
 *   - Chat history caching per target (switches between agents without losing context)
 *   - Auto-save chat history to in-memory cache
 *   - Agent selection dropdown with profile preview
 *
 * Props:
 *   @prop {string} reportId - The report ID for Report Agent chat
 *   @prop {string} simulationId - The simulation ID for agent interviews
 *
 * Emits:
...
/** @type {import('vue').Ref<string>} Active tab ('chat' or 'survey') */
const activeTab = ref('chat')
/** @type {import('vue').Ref<string>} Chat target ('report_agent' or 'agent') */
const chatTarget = ref('report_agent')
/** @type {import('vue').Ref<boolean>} Whether the agent selection dropdown is open */
const showAgentDropdown = ref(false)
/** @type {import('vue').Ref<Object|null>} Currently selected agent profile */
const selectedAgent = ref(null)
/** @type {import('vue').Ref<number|null>} Currently selected agent index (agent_id) */
const selectedAgentIndex = ref(null)
/** @type {import('vue').Ref<boolean>} Whether to show full agent profile details */
const showFullProfile = ref(true)
/** @type {import('vue').Ref<boolean>} Whether to show agent tool descriptions */
const showToolsDetail = ref(true)

// ─── Chat State ───────────────────────────────────────────────────────────────

/** @type {import('vue').Ref<string>} Current chat input text */
const chatInput = ref('')
/** @type {import('vue').Ref<Object[]>} Current chat history messages */
const chatHistory = ref([])
/** @type {import('vue').Ref<Object>} Chat history cache per target: { 'report_agent': [], 'agent_0': [], ... } */
const chatHistoryCache = ref({})
/** @type {import('vue').Ref<boolean>} Whether a chat message is being sent */
const isSending = ref(false)
/** @type {import('vue').Ref<HTMLElement|null>} Reference to the chat messages container for auto-scroll */
const chatMessages = ref(null)
/** @type {import('vue').Ref<HTMLInputElement|null>} Reference to the chat input element */
const chatInputRef = ref(null)

// ─── Survey State ─────────────────────────────────────────────────────────────

/** @type {import('vue').Ref<Set<number>}} Set of selected agent indices for multi-agent survey */
const selectedAgents = ref(new Set())
/** @type {import('vue').Ref<string>} Survey question text */
const surveyQuestion = ref('')
/** @type {import('vue').Ref<Object[]>} Survey results from interviewed agents */
const surveyResults = ref([])
/** @type {import('vue').Ref<boolean>} Whether a survey is in progress */
const isSurveying = ref(false)

// ─── Report Data ──────────────────────────────────────────────────────────────

/** @type {import('vue').Ref<Object|null>} Report outline with title, summary, and sections */
const reportOutline = ref(null)
/** @type {import('vue').Ref<Object>} Generated section content keyed by section index */
const generatedSections = ref({})
/** @type {import('vue').Ref<Set<number>}} Set of collapsed section indices */
const collapsedSections = ref(new Set())
/** @type {import('vue').Ref<number|null>} Currently generating section index */
const currentSectionIndex = ref(null)
/** @type {import('vue').Ref<Object[]>} Agent profiles loaded from the simulation */
const profiles = ref([])

// ─── Helper Methods ───────────────────────────────────────────────────────────

/** Check if a report section has been generated */
...
/** Emit a log entry to the parent component */
const addLog = (msg) => {
  emit('add-log', msg)
}

/** Toggle collapse/expand of a report section */
...
/** Switch the chat target (report_agent or agent) */
...
/** Save current chat history to the in-memory cache (keyed by target) */
...
/** Switch to Report Agent chat, saving and restoring chat history */
...
/** Switch to the multi-agent survey tab */
...
/** Select an agent for individual chat, saving and restoring chat history */
...
/**
 * Send a chat message to the current target (Report Agent or individual agent).
 * Routes to the appropriate API method and handles the response.
 */
...
/** Send a message to the Report Agent API with chat history context */
...
/** Send an interview message to a specific simulated agent with conversation history */
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert object dictionary to array, preferring reddit platform responses
