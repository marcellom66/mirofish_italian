        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched while running and do not support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
<script setup>
/**
 * HistoryDatabase.vue — Simulation history card gallery
 *
 * Displays past simulations as interactive cards with two layout modes:
 *   - Collapsed: fan-shaped card stack (default)
 *   - Expanded: grid layout (triggered by IntersectionObserver when scrolled into view)
 *
 * Features:
 *   - Animated expand/collapse transitions with debounce to prevent flickering
 *   - Detail modal with navigation to Graph Building, Environment Setup, and Report pages
 *   - Auto-reload when navigating back to the home page
 *   - IntersectionObserver-based visibility detection for layout switching
 */
...
/** @type {import('vue').Ref<Object[]>} List of simulation history entries */
const projects = ref([])
/** @type {import('vue').Ref<boolean>} Whether history data is being loaded */
const loading = ref(true)
/** @type {import('vue').Ref<boolean>} Whether cards are in expanded (grid) layout */
const isExpanded = ref(false)
/** @type {import('vue').Ref<number|null>} Index of the card currently being hovered */
const hoveringCard = ref(null)
/** @type {import('vue').Ref<HTMLElement|null>} Reference to the container element for IntersectionObserver */
const historyContainer = ref(null)
/** @type {import('vue').Ref<Object|null>} Currently selected project for the detail modal */
const selectedProject = ref(null)

// ─── Animation & Observer State ───────────────────────────────────────────────

/** @type {IntersectionObserver|null} Observer for detecting when the section is in viewport */
let observer = null
/** @type {boolean} Lock to prevent layout animation flickering during transitions */
let isAnimating = false
/** @type {number|null} Debounce timer ID for expand/collapse state changes */
let expandDebounceTimer = null
/** @type {boolean|null} Pending target state recorded during animation lock */
let pendingState = null

// ─── Card Layout Configuration ────────────────────────────────────────────────

/** Number of cards per row in expanded grid mode */
...
// Dynamically calculate container height style based on card count and layout mode
...
/**
 * Calculate inline styles for a card based on its index and current layout mode.
 * In expanded mode: positions cards in a centered grid.
 * In collapsed mode: positions cards in a fan-shaped stack with rotation and scale.
 * @param {number} index - Card index in the projects array
 * @returns {Object} CSS transform, zIndex, opacity, and transition styles
 */
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for current row, ensure centering per row
...
/**
 * Determine the CSS class for a simulation's progress indicator.
 * @param {Object} simulation - Simulation object with current_round and total_rounds
 * @returns {'not-started'|'completed'|'in-progress'} Progress state class
 */
...
/** Extract a short title from the simulation requirement (first 20 chars) */
...
/** Format simulation_id for display (e.g. "sim_abc123" → "SIM_ABC123") */
...
/** Format round progress as "current/total rounds" or "Not Started" */
 ...
 * @returns {'pdf'|'doc'|'xls'|'ppt'|'txt'|'code'|'img'|'zip'|'other'} File type category
 */
...
/** Get the uppercase file extension label (e.g. "PDF", "TXT") */
...
/** Open the detail modal for a simulation project */
...
/** Navigate to the Graph Building page (Step 1) for the selected project */
const goToProject = () => {
...
/** Navigate to the Environment Setup page (Step 2) for the selected simulation */
...
/** Navigate to the Analysis Report page (Step 4) for the selected report */
const goToReport = () => {
...
/** Fetch the last 20 simulation history entries from the API */
...
/**
 * Initialize the IntersectionObserver to detect when the history section
 * enters/exits the viewport. Uses debounce and animation locking to prevent
 * rapid flickering between expanded and collapsed states.
 */
const initObserver = () => {
  if (observer) {
    observer.disconnect()
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Shorter delay for expanding (50ms), longer delay for collapsing (200ms) for stability
...
// Reload history data when navigating back to the home page
...
onMounted(async () => {
  // Ensure DOM rendering is complete before loading data and initializing observer
  await nextTick()
  await loadHistory()
  setTimeout(() => { initObserver() }, 100)
})

// If using keep-alive, reload data when component is reactivated
onActivated(() => {
  loadHistory()
})

onUnmounted(() => {
  // Clean up IntersectionObserver and debounce timer to prevent memory leaks
  if (observer) {
    observer.disconnect()
    observer = null
  }
