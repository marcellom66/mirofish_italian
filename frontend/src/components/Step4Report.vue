<template>
  <div class="report-panel">
    <!-- Quick Test Results (shown when this simulation was started from a Quick Test template) -->
    <QuickTestResults
      v-if="quickTestTemplateId"
      :loading="quickTestLoading"
      :error="quickTestError"
      :analysisData="quickTestData"
      @retry="runQuickTestEvaluation"
    />
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When this is the final answer, show special prompt -->
...
<script setup>
/**
 * Step4Report.vue — Analysis Report viewer (Step 4 of the pipeline)
 *
 * Displays the generated simulation analysis report with:
 *   - Report outline with collapsible sections (streamed from the Report Agent)
 *   - Markdown rendering for generated section content
 *   - Quick Test evaluation results (if simulation was started from a template)
 *   - Agent action log panel with platform filtering
 *   - Console log viewer for debugging
 *   - Navigation to the Deep Interaction page (Step 5)
 *
 * Props:
 *   @prop {string} reportId - The report ID to display
 *   @prop {string} simulationId - The associated simulation ID
 *   @prop {Array} systemLogs - Array of {time, msg} log entries
 */
import { ref, computed, watch, onMounted, onUnmounted, nextTick, h, reactive } from 'vue'
import { useRouter } from 'vue-router'
import { useI18n } from 'vue-i18n'
import { getAgentLog, getConsoleLog } from '../api/report'
import { interviewAllAgents } from '../api/simulation'
import { getPendingUpload } from '../store/pendingUpload'
import QuickTestResults from './QuickTestResults.vue'
...
/** @type {import('vue').Ref<string|null>} Quick-test template ID (if applicable) */
const quickTestTemplateId = ref(null)
/** @type {import('vue').Ref<Object|null>} Quick-test template configuration with evaluation prompt */
const quickTestTemplateConfig = ref(null)
/** @type {import('vue').Ref<boolean>} Whether quick-test evaluation is loading */
const quickTestLoading = ref(false)
/** @type {import('vue').Ref<string|null>} Quick-test evaluation error message */
const quickTestError = ref(null)
/** @type {import('vue').Ref<Object|null>} Quick-test analysis data with responses array */
const quickTestData = ref(null)

/**
 * Parse an agent's interview response text into a structured format.
 * Tries JSON extraction first, falls back to keyword-based sentiment detection.
 * @param {string} text - Raw agent response text
 * @returns {{ sentiment: string, score: number, reason: string, category: string }}
 */
function parseAgentResponse(text) {
  try {
    ...
    if (jsonMatch) {
      ...
      return {
        ...
        reason: parsed.reason || parsed.motivo || '',
        category: parsed.category || parsed.categoria || 'Unknown'
      }
    }
  } catch {}
  // Keyword fallback
  ...
  let score = 0
  if (lower.includes('favorable') || lower.includes('favorevole') || lower.includes('positive') || lower.includes('positivo') || lower.includes('support') || lower.includes('agree')) {
    sentiment = 'favorable'; score = 5
  } else if (lower.includes('unfavorable') || lower.includes('contrario') || lower.includes('negative') || lower.includes('negativo') || lower.includes('oppose') || lower.includes('disagree')) {
    sentiment = 'unfavorable'; score = -5
  }
  return { sentiment, score, reason: text.substring(0, 100), category: 'Unknown' }
}

/** Normalize a sentiment string to one of: favorable, unfavorable, neutral */
function normalizeSentiment(s) {
  if (!s) return 'neutral'
  const lower = s.toLowerCase()
  if (lower.includes('favor') || lower === 'positivo' || lower === 'positive') return 'favorable'
  if (lower.includes('unfavor') || lower === 'contrario' || lower === 'negativo' || lower === 'negative') return 'unfavorable'
  return 'neutral'
}

/**
 * Run the quick-test evaluation by interviewing all agents with the template's evaluation prompt.
 * Parses responses into structured sentiment data for the QuickTestResults component.
 */
async function runQuickTestEvaluation() {
  if (!props.simulationId || !quickTestTemplateConfig.value) return
  quickTestLoading.value = true
  quickTestError.value = null
  try {
    const res = await interviewAllAgents({
      simulation_id: props.simulationId,
      prompt: quickTestTemplateConfig.value.evaluationPrompt
    })
    const interviews = res.data?.interviews || res.data?.results || res.data || []
    const responses = (Array.isArray(interviews) ? interviews : []).map((item, i) => {
      const text = item.response || item.reply || item.content || ''
      const parsed = parseAgentResponse(text)
      return { agentId: item.agent_id || `Agent ${i + 1}`, ...parsed }
    })
    quickTestData.value = { responses }
  } catch (err) {
    quickTestError.value = `Failed to interview agents: ${err.message || 'Unknown error'}`
  } finally {
    quickTestLoading.value = false
  }
}

// On mount, check if this simulation was started from a Quick Test template
onMounted(() => {
  const pending = getPendingUpload()
  if (pending.templateId) {
    quickTestTemplateId.value = pending.templateId
    quickTestTemplateConfig.value = pending.templateConfig
    runQuickTestEvaluation()
  } else {
    const storedId = sessionStorage.getItem('quickTestTemplateId')
    if (storedId) {
      quickTestTemplateId.value = storedId
      try {
        quickTestTemplateConfig.value = JSON.parse(sessionStorage.getItem('quickTestTemplateConfig'))
      } catch {}
      if (quickTestTemplateConfig.value) {
        runQuickTestEvaluation()
      }
    }
  }
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:\u5206\u6790\u95ee\u9898|Analysis Question|Domanda di Analisi):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract prediction scenario
    const reqMatch = text.match(/(?:\u9884\u6d4b\u573a\u666f|Prediction Scenario|Scenario di Previsione):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract statistics - match "related prediction facts: X items" format
    const factMatch = text.match(/(?:\u76f8\u5173\u9884\u6d4b\u4e8b\u5b9e|Related Prediction Facts|Fatti di Previsione Correlati):\s*(\d+)/)
    const entityMatch = text.match(/(?:\u6d89\u53ca\u5b9e\u4f53|Involved Entities|Entita Coinvolte):\s*(\d+)/)
    const relMatch = text.match(/(?:\u5173\u7cfb\u94fe|Relationship Chains|Catene di Relazioni):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-questions - full extraction, no limit on count
    const subQSection = text.match(/### (?:\u5206\u6790\u7684\u5b50\u95ee\u9898|Analyzed Sub-Questions|Sotto-domande Analizzate)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract key facts - full extraction, no limit on count
    const factsSection = text.match(/### (?:\u3010\u5173\u952e\u4e8b\u5b9e\u3011|\[Key Facts\]|\[Fatti Chiave\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    const entitySection = text.match(/### (?:\u3010\u6838\u5fc3\u5b9e\u4f53\u3011|\[Core Entities\]|\[Entita Principali\])\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const summaryMatch = block.match(/(?:\u6458\u8981|Summary|Sommario):\s*"?(.+?)"?(?:\n|$)/)
        const relatedMatch = block.match(/(?:\u76f8\u5173\u4e8b\u5b9e|Related Facts|Fatti Correlati):\s*(\d+)/)
        return {
    ...
    // Extract relationship chains - full extraction, no limit on count
    const relSection = text.match(/### (?:\u3010\u5173\u7cfb\u94fe\u3011|\[Relationship Chains\]|\[Catene di Relazioni\])\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Extract query
    const queryMatch = text.match(/(?:\u67e5\u8be2|Query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract statistics
    const nodesMatch = text.match(/(?:\u603b\u8282\u70b9\u6570|Total Nodes|Nodi Totali):\s*(\d+)/)
    const edgesMatch = text.match(/(?:\u603b\u8fb9\u6570|Total Edges|Archi Totali):\s*(\d+)/)
    const activeMatch = text.match(/(?:\u5f53\u524d\u6709\u6548\u4e8b\u5b9e|Currently Active Facts|Fatti Attivi Correnti):\s*(\d+)/)
    const histMatch = text.match(/(?:\u5386\u53f2\/\u8fc7\u671f\u4e8b\u5b9e|Historical\/Expired Facts|Fatti Storici\/Scaduti):\s*(\d+)/)
    ...
    // Extract currently active facts - full extraction, no limit on count
    const activeSection = text.match(/### (?:\u3010\u5f53\u524d\u6709\u6548\u4e8b\u5b9e\u3011|\[Currently Active Facts\]|\[Fatti Attivi Correnti\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extract historical/expired facts - full extraction, no limit on count
    const histSection = text.match(/### (?:\u3010\u5386\u53f2\/\u8fc7\u671f\u4e8b\u5b9e\u3011|\[Historical\/Expired Facts\]|\[Fatti Storici\/Scaduti\])[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    // Extract involved entities - full extraction, no limit on count
    const entitySection = text.match(/### (?:\u3010\u6d89\u53ca\u5b9e\u4f53\u3011|\[Involved Entities\]|\[Entita Coinvolte\])\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const topicMatch = text.match(/\*\*(?:\u91c7\u8bbf\u4e3b\u9898|Interview Topic|Tema Intervista):\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extract interview count (e.g. "5 / 9 simulated Agents")
    const countMatch = text.match(/\*\*(?:\u91c7\u8bbf\u4eba\u6570|Interviewed Count|Numero Intervistati):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:\u91c7\u8bbf\u5bf9\u8c61\u9009\u62e9\u7406\u7531|Selection Reason|Motivo Selezione)\n([\s\S]*?)(?=\n---\n|\n### (?:\u91c7\u8bbf\u5b9e\u5f55|Interview Transcript|Trascrizione Intervista))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **alumni_345 (index=1)**: As an alumni...
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:\u9009\u62e9|select|seleziona)\s*([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/i)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^(?:\u672a\u9009|\u7efc\u4e0a|\u6700\u7ec8\u9009\u62e9|not selected|overall|final selection)/i)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract bio
      const bioMatch = block.match(/_(?:\u7b80\u4ecb|Bio|Profilo):\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*(?:\u5173\u952e\u5f15\u8a00|Key Quotes|Citazioni Chiave)|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/(?:\u3010Twitter\u5e73\u53f0\u56de\u7b54\u3011|\[Twitter Answer\]|\[Risposta Twitter\])\n?([\s\S]*?)(?=(?:\u3010Reddit\u5e73\u53f0\u56de\u7b54\u3011|\[Reddit Answer\]|\[Risposta Reddit\])|$)/)
        const redditMatch = answerText.match(/(?:\u3010Reddit\u5e73\u53f0\u56de\u7b54\u3011|\[Reddit Answer\]|\[Risposta Reddit\])\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic (backward compatible: only one platform marker present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer available, copy as default display only if not placeholder text
          if (interview.redditAnswer && interview.redditAnswer !== '(no reply for this platform)' && interview.redditAnswer !== '(nessuna risposta per questa piattaforma)' && interview.redditAnswer !== '\uFF08\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D\uFF09') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(no reply for this platform)' && interview.twitterAnswer !== '(nessuna risposta per questa piattaforma)' && interview.twitterAnswer !== '\uFF08\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D\uFF09') {
      ...
      const quotesMatch = block.match(/\*\*(?:\u5173\u952e\u5f15\u8a00|Key Quotes|Citazioni Chiave):\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
    ...
    const summaryMatch = text.match(/### (?:\u91c7\u8bbf\u6458\u8981\u4e0e\u6838\u5fc3\u89c2\u70b9|Interview Summary and Key Takeaways|Sintesi Intervista e Punti Chiave)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:\u641c\u7d22\u67e5\u8be2|Search Query|Query di Ricerca):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract result count
    const countMatch = text.match(/(?:\u627e\u5230|found|trovati)\s*(\d+)\s*(?:\u6761|items|elementi)/i)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extract related facts - full extraction, no limit on count
    const factsSection = text.match(/### (?:\u76f8\u5173\u4e8b\u5b9e|Related Facts|Fatti Correlati):\n([\s\S]*)$/)
    if (factsSection) {
      const lines = factsSection[1].split('\n').filter(l => l.match(/^\d+\./))
    ...
    const edgesSection = text.match(/### (?:\u76f8\u5173\u8fb9|Related Edges|Archi Correlati):\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    const nodesSection = text.match(/### (?:\u76f8\u5173\u8282\u70b9|Related Nodes|Nodi Correlati):\n([\s\S]*?)(?=\n###|$)/)
    if (nodesSection) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(no reply for this platform)' || t === '(nessuna risposta per questa piattaforma)' || t === '[no reply]' || t === '[nessuna risposta]' || t === '\uFF08\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D\uFF09' || t === '(\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D)' || t === '[\u65e0\u56de\u590d]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" format (legacy CN supported via unicode pattern below)
      // 2. "1. " or "\n1. " (number+dot, old format compatible)
      let matches = []
      let match

      // Prefer legacy CN question-number format first
      const cnPattern = /(?:^|[\r\n]+)\u95ee\u9898(\d+)[\uff1a:]\s*/g
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), since section title is already displayed in outer layer
  ...
  // Clean up <p><br> immediately followed by block-level elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph start tags immediately after block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single-item <ol> are separated by paragraph content, maintain incrementing numbering
...
const extractFinalContent = (response) => {
  ...
  // Try legacy Chinese "Final Answer" marker via unicode escape
