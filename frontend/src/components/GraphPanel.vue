...
<script setup>
/**
 * GraphPanel.vue — Interactive knowledge graph visualization
 *
 * Renders a force-directed graph using D3.js with:
 *   - Draggable nodes with entity type coloring
 *   - Labeled edges with toggle visibility
 *   - Node detail panel on click (attributes, connections, self-loops)
 *   - Entity type legend with color coding
 *   - Post-simulation hint overlay
 *   - Responsive SVG that adapts to container size
 *
 * Props:
 *   @prop {Object|null} graphData - Graph data with nodes[] and edges[] arrays
 *   @prop {boolean} loading - Whether graph data is being loaded
 *   @prop {number} currentPhase - Current pipeline phase (affects UI hints)
 *   @prop {boolean} isSimulating - Whether a simulation is currently running
 *
 * Emits:
 *   @emit {void} refresh - Request to reload graph data
 *   @emit {void} toggle-maximize - Request to toggle panel maximization
 */
...
/** @type {import('vue').Ref<HTMLElement|null>} Reference to the graph container for sizing */
const graphContainer = ref(null)
/** @type {import('vue').Ref<SVGSVGElement|null>} Reference to the SVG element for D3 rendering */
const graphSvg = ref(null)
/** @type {import('vue').Ref<Object|null>} Currently selected graph node for the detail panel */
const selectedItem = ref(null)
/** @type {import('vue').Ref<boolean>} Whether edge labels are visible */
const showEdgeLabels = ref(true)
/** @type {import('vue').Ref<Set<string>}} Set of expanded self-loop node IDs */
const expandedSelfLoops = ref(new Set())
/** @type {import('vue').Ref<boolean>} Whether to show the post-simulation finished hint */
const showSimulationFinishedHint = ref(false)
/** @type {import('vue').Ref<boolean>} Track whether simulation was running (for transition detection) */
const wasSimulating = ref(false)

/** Dismiss the post-simulation finished hint */
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Watch isSimulating to detect simulation end and show hint
...
/** Toggle expand/collapse of a self-loop node in the detail panel */
...
/** Compute unique entity types with colors and counts for the legend */
const entityTypes = computed(() => {
  ...
  props.graphData.nodes.forEach(node => {
    const type = node.labels?.find(l => l !== 'Entity') || 'Entita'
    if (!typeMap[type]) {
...
/** Close the node detail panel and reset expanded self-loop state */
const closeDetailPanel = () => {
  selectedItem.value = null
  expandedSelfLoops.value = new Set() // Reset expanded state
}

/** D3 force simulation instance (stored for cleanup on re-render) */
let currentSimulation = null
/** D3 reference to edge label text elements */
let linkLabelsRef = null
/** D3 reference to edge label background rectangles */
let linkLabelBgRef = null

/**
 * Render the force-directed graph using D3.js.
 * Clears previous rendering, creates nodes/edges, applies force simulation,
 * and sets up drag, click, and zoom interactions.
 */
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Curvature range increases with edge count; more edges means wider range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is opposite to normalized direction, flip curvature
      // This ensures all edges are distributed in the same reference frame, preventing overlap due to direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center so isolated node clusters gather in the center area
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the right side of the node, loop around and back
    ...
    // Perpendicular offset to the link direction, proportional to distance, ensuring curves are clearly visible
    // More edges means a larger offset-to-distance ratio
  ...
  // Link labels background (white background makes text more readable)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging has started (movement exceeds threshold)
