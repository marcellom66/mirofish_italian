      ...
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <div class="platform-stats">
            <span class="stat">
              ...
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ displayTotalRounds }}</span></span>
            </span>
            <span class="stat">
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('step3.round') }}</span>
              <span class="stat-value mono">{{ runStatus.reddit_current_round || 0 }}<span class="stat-total">/{{ displayTotalRounds }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('step3.elapsedTime') }}</span>
...
<script setup>
/**
 * Step3Simulation.vue — Simulation Run control panel (Step 3 of the pipeline)
 *
 * Controls the execution of a multi-platform social simulation with:
 *   - Start/Stop simulation controls
 *   - Real-time status monitoring for Twitter (Info Plaza) and Reddit (Topic Community)
 *   - Live action feed with incremental accumulation and deduplication
 *   - Elapsed time estimation based on rounds × minutes per round
 *   - Post-simulation report generation
 *
 * Props:
 *   @prop {string} simulationId - The simulation ID to control
 *   @prop {number} maxRounds - Maximum number of simulation rounds
 *   @prop {number} minutesPerRound - Simulated minutes per round (default: 30)
 *   @prop {Object|null} projectData - Project data
 *   @prop {Object|null} graphData - Graph data
 *   @prop {Array} systemLogs - Array of {time, msg} log entries
 *
 * Emits:
 *   @emit {void} go-back - Navigate back to Step 2
 *   @emit {void} next-step - Advance to Step 4 (report)
...
/** @type {import('vue').Ref<boolean>} Whether a report generation request is in progress */
const isGeneratingReport = ref(false)
/** @type {import('vue').Ref<number>} Current phase (0=not started, 1=running, 2=completed) */
const phase = ref(0)
/** @type {import('vue').Ref<boolean>} Whether a start request is in progress */
const isStarting = ref(false)
/** @type {import('vue').Ref<boolean>} Whether a stop request is in progress */
const isStopping = ref(false)
/** @type {import('vue').Ref<string|null>} Error message from start attempt */
const startError = ref(null)
/** @type {import('vue').Ref<Object>} Current simulation run status from polling */
const runStatus = ref({})
/** @type {import('vue').Ref<Object[]>} All simulation actions (incrementally accumulated) */
const allActions = ref([])
/** @type {import('vue').Ref<Set<string>}} Set of action IDs for deduplication */
const actionIds = ref(new Set())
/** @type {import('vue').Ref<HTMLElement|null>} Reference to the scroll container for auto-scroll */
const scrollContainer = ref(null)

// ─── Computed Properties ──────────────────────────────────────────────────────

/** Actions in chronological order (newest at bottom) */
const chronologicalActions = computed(() => {
  return allActions.value
})

/** Count of Twitter-platform actions */
const twitterActionsCount = computed(() => {
  return allActions.value.filter(a => a.platform === 'twitter').length
})

/** Count of Reddit-platform actions */
const redditActionsCount = computed(() => {
  return allActions.value.filter(a => a.platform === 'reddit').length
})

/** Display total rounds, keeping denominator coherent even with stale frontend state */
const displayTotalRounds = computed(() => {
  const backendTotal = Number(runStatus.value.total_rounds || 0)
  const uiMax = Number(props.maxRounds || 0)
  const twitterRound = Number(runStatus.value.twitter_current_round || 0)
  const redditRound = Number(runStatus.value.reddit_current_round || 0)
  const safeTotal = Math.max(backendTotal, uiMax, twitterRound, redditRound)
  return safeTotal > 0 ? safeTotal : '-'
})
...
/** Convert rounds to simulated hours */
const roundToHours = (currentRound) => {
  if (!currentRound || currentRound <= 0) return 0
  return Math.floor((currentRound * props.minutesPerRound) / 60)
}
...
/** Reset all simulation state (used when restarting) */
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
/** Start the simulation via API and begin polling for status updates */
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.twitter_current_round > prevTwitterRound.value) {
        const twitterHours = roundToHours(data.twitter_current_round)
        addLog(`[Plaza] R${data.twitter_current_round}/${displayTotalRounds.value} | T:${twitterHours}h | A:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        const redditHours = roundToHours(data.reddit_current_round)
        addLog(`[Community] R${data.reddit_current_round}/${displayTotalRounds.value} | T:${redditHours}h | A:${data.reddit_actions_count}`)
