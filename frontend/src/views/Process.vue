  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        ...
        <div class="step-name">{{ $t('process.graphConstruction') }}</div>
      </div>
    ...
    <div class="main-content">
      <!-- Left: Real-time graph visualization -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-if="graphData" class="graph-view">
            ...
            <div v-if="selectedItem" class="detail-panel">
              ...
              <div v-else class="detail-content">
                <!-- {{ $t('process.relationship') }} display -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.waitingDesc') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.buildingGraphWaiting') }}</p>
            <p class="waiting-hint">{{ $t('process.dataWillAppearShortly') }}</p>
          </div>
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  {{ $t('process.ontologyApiDescription') }}
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waiting') }}...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.graphConstruction') }}</div>
          ...
          <div class="next-step-section" v-if="currentPhase >= 2">
            <button class="next-step-btn" @click="goToNextStep" :disabled="currentPhase < 2">
              {{ $t('process.enterEnvironmentSetup') }}
...
const statusText = computed(() => {
  ...
  if (currentPhase.value >= 2) return t('process.buildCompleteStatus')
  if (currentPhase.value === 1) return t('process.buildingGraphStatus')
  if (currentPhase.value === 0) return t('process.generatingOntologyStatus')
...
const goToNextStep = () => {
  ...
  alert(t('process.environmentSetupUnderDevelopment'))
}
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = `${t('process.projectInitializationFailed')}: ${err.message || t('step1.unknownError')}`
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = `${t('process.failedToLoadProject')}: ${err.message || t('step1.unknownError')}`
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: t('process.startingGraphConstruction')
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = t('process.graphConstructionTaskStarted')
      ...
      error.value = response.error || t('process.failedToStartGraphConstruction')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = `${t('process.failedToStartGraphConstruction')}: ${err.message || t('step1.unknownError')}`
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        console.log(`${t('process.fetchingGraphData')}:`, newNodeCount, `${t('process.edges')}:`, newData.edge_count || newData.edges?.length || 0)
    ...
    console.log(`${t('process.graphDataFetch')}:`, err.message || t('process.notReady'))
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log(t('process.graphConstructionCompleteLoading'))
        ...
        if (projectResponse.success) {
          ...
          // Finally load the complete graph data
          if (projectResponse.data.graph_id) {
            console.log('Loading full graph:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log(t('process.graphLoadingComplete'))
          }
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = `${t('process.graphConstructionFailed')}: ${task.error || t('step1.unknownError')}`
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  if (!graphSvg.value || !graphData.value) {
    console.log(t('process.cannotRenderSvgOrDataMissing'))
    return
  }
  
  const container = graphContainer.value
  if (!container) {
    console.log(t('process.cannotRenderContainerMissing'))
    return
  }
  ...
  if (width <= 0 || height <= 0) {
    console.log(t('process.cannotRenderInvalidDimensions'), width, height)
    return
  }
  ...
  if (nodesData.length === 0) {
    console.log(t('process.noNodesToRender'))
    // Show empty state
    svg.append('text')
      .attr('x', width / 2)
      .attr('y', height / 2)
  ...
  const edges = edgesData
    ...
    .map(e => ({
      ...
      rawData: {
        ...e,
        source_name: nodeMap[e.source_node_uuid]?.name || t('quickTest.unknown'),
        target_name: nodeMap[e.target_node_uuid]?.name || t('quickTest.unknown')
      }
    }))
  
  console.log(`${t('process.nodes')}:`, nodes.length, `${t('process.edges')}:`, edges.length)
