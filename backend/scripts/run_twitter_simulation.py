"""
OASIS Twitter simulation preset script / Script preset simulazione OASIS Twitter.
...
Usage / Utilizzo:
        ...
        python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # close immediately / chiudi subito
"""
...
class TwitterSimulationRunner:
    """Twitter simulation runner / Esecutore simulazione Twitter."""
    
    # Available Twitter actions (INTERVIEW is manual-only) / Azioni Twitter disponibili (INTERVIEW solo manuale)
    AVAILABLE_ACTIONS = [
    ...
    def _get_profile_path(self) -> str:
        """Get profile file path (Twitter uses CSV) / Ottieni path file profilo (Twitter usa CSV)."""
    ...
    def _get_active_agents_for_round(
        ...
        Args:
            env: OASIS environment / ambiente OASIS
            current_hour: current simulation hour (0-23) / ora simulazione corrente (0-23)
            round_num: current round / round corrente
            
        Returns:
            active agent list / lista agenti attivi
        """
        time_config = self.config.get("time_config", {})
        agent_configs = self.config.get("agent_configs", [])
        
        # base number of active agents / numero base di agenti attivi
        base_min = time_config.get("agents_per_hour_min", 5)
        base_max = time_config.get("agents_per_hour_max", 20)
        
        # adjust by time window / regola in base alla fascia oraria
        ...
        # compute activation probability from each agent config / calcola la probabilita di attivazione dalla config di ogni agente
        candidates = []
        for cfg in agent_configs:
            ...
            # check whether it is within active hours / verifica se e nelle ore attive
            if current_hour not in active_hours:
                continue
            
            # compute probability from activity level / calcola la probabilita dal livello di attivita
        ...
        # convert to Agent objects / converti in oggetti Agent
        active_agents = []
    ...
    async def run(self, max_rounds: int = None):
        ...
        """
        print("=" * 60)
        print("OASIS Twitter simulation / simulazione OASIS Twitter")
        ...
        # load time configuration / carica configurazione temporale
        time_config = self.config.get("time_config", {})
        total_hours = time_config.get("total_simulation_hours", 72)
        minutes_per_round = time_config.get("minutes_per_round", 30)
        
        # compute total rounds / calcola round totali
        ...
        # load Agent graph / carica grafo Agent
        ...
        # database path / percorso database
        ...
        for round_num in range(total_rounds):
            # compute current simulated time / calcola tempo simulato corrente
            simulated_minutes = round_num * minutes_per_round
            simulated_hour = (simulated_minutes // 60) % 24
            simulated_day = simulated_minutes // (60 * 24) + 1
            
            # get active agents for this round / ottieni agenti attivi per questo round
            ...
            # build actions / costruisci azioni
            actions = {
                agent: LLMAction()
                for _, agent in active_agents
            }
            
            # execute actions / esegui azioni
            await self.env.step(actions)
            
            # print progress / stampa avanzamento
...
async def main():
    parser = argparse.ArgumentParser(description='OASIS Twitter simulation / simulazione OASIS Twitter')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,
