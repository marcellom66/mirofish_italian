"""
Action logger / Logger azioni.
Records each agent action during OASIS simulations for backend monitoring /
Registra ogni azione degli agenti durante le simulazioni OASIS per il monitoraggio backend.

Log structure / Struttura log:
    sim_xxx/
    ├── twitter/
    │   └── actions.jsonl    # Twitter platform action log / Log azioni piattaforma Twitter
    ├── reddit/
    │   └── actions.jsonl    # Reddit platform action log / Log azioni piattaforma Reddit
    ├── simulation.log       # Main simulation process log / Log processo principale simulazione
...
class PlatformActionLogger:
    """Single-platform action logger / Logger azioni per singola piattaforma."""
    
    def __init__(self, platform: str, base_dir: str):
        """
        Initialize logger / Inizializza il logger.
        
        Args:
            platform: platform name (twitter/reddit) / nome piattaforma (twitter/reddit)
            base_dir: base path of simulation directory / percorso base directory simulazione
        """
    ...
    def _ensure_dir(self):
        """Ensure directory exists / Assicura che la directory esista."""
    ...
    def log_action(
        ...
        """Log one action / Registra una azione."""
        entry = {
            "round": round_num,
    ...
    def log_round_start(self, round_num: int, simulated_hour: int):
        """Log round start / Registra inizio round."""
        entry = {
            "round": round_num,
            "timestamp": datetime.now().isoformat(),
    ...
    def log_round_end(self, round_num: int, actions_count: int):
        """Log round end / Registra fine round."""
        entry = {
            "round": round_num,
            "timestamp": datetime.now().isoformat(),
    ...
    def log_simulation_start(self, config: Dict[str, Any]):
        """Log simulation start / Registra inizio simulazione."""
        entry = {
            "timestamp": datetime.now().isoformat(),
    ...
    def log_simulation_end(self, total_rounds: int, total_actions: int):
        """Log simulation end / Registra fine simulazione."""
        entry = {
            "timestamp": datetime.now().isoformat(),
...
class SimulationLogManager:
    """
    Simulation log manager / Gestore log simulazione.
    Manages all log files with platform separation /
    Gestisce tutti i file di log separati per piattaforma.
    """
    
    def __init__(self, simulation_dir: str):
        """
        Initialize log manager / Inizializza gestore log.
        ...
        # Set up main log / Configura log principale
        self._setup_main_logger()
    
    def _setup_main_logger(self):
        """Set up main simulation log / Configura log principale simulazione."""
        log_path = os.path.join(self.simulation_dir, "simulation.log")
        
        # Create logger / Crea logger
    ...
    def get_twitter_logger(self) -> PlatformActionLogger:
        """Get Twitter platform logger / Ottieni logger piattaforma Twitter."""
    ...
    def get_reddit_logger(self) -> PlatformActionLogger:
        """Get Reddit platform logger / Ottieni logger piattaforma Reddit."""
    ...
    def log(self, message: str, level: str = "info"):
        """Log main message / Registra messaggio principale."""
...
# ============ Backward-compatible legacy interface / Interfaccia legacy compatibile ============

class ActionLogger:
    """
    Action logger (legacy compatibility mode) /
    Logger azioni (modalita compatibilita legacy).
    Recommended: use SimulationLogManager instead /
    Consigliato: usare SimulationLogManager al suo posto.
    """
...
# Global logger instance (legacy compatible) / Istanza logger globale (compatibile legacy)
_global_logger: Optional[ActionLogger] = None


def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """Get global logger instance (legacy compatible) / Ottieni istanza logger globale (compatibile legacy)."""
