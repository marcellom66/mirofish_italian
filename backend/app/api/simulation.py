...
def create_simulation():
    ...
    Note: parameters such as max_rounds are generated intelligently by the LLM and do not need to be set manually.
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # Detailed logging
        logger.debug(f"Checking simulation preparation state: {simulation_id}, status={status}, config_generated={config_generated}")
        ...
        # - running: currently running, preparation was already done
        # - completed: run finished, preparation was already done
        # - stopped: stopped, preparation was already done
...
def prepare_simulation():
    """
    Prepare simulation environment (async task, all parameters generated intelligently by LLM).
    ...
    Features:
    - Automatically detects completed preparation to avoid regeneration
    - If already prepared, returns existing result directly
    ...
    Returns:
        {
            ...
            "data": {
                "simulation_id": "sim_xxxx",
                "task_id": "task_xxxx",           // returned when a new task is started
    ...
    try:
        ...
        # Check if preparation is already complete (avoid regeneration)
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Project is missing simulation_requirement description"
            }), 400
        ...
        try:
            ...
            # Save entity count to state (available for frontend immediately)
...
def get_prepare_status():
    ...
    1. Query in-progress task progress by task_id
    2. Check whether preparation is already complete by simulation_id
    ...
    try:
        ...
        if not task:
            # Task not found; if simulation_id is provided, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates through the reports directory and finds reports matching simulation_id.
    ...
    # __file__ is app/api/simulation.py, need to go two levels up to backend/
...
def get_simulation_history():
    ...
    Used for home page history display; returns simulation list enriched with project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read actual total_rounds set by user from run_state.json)
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation config in real time (for live progress viewing during generation).
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (full config generated intelligently by LLM).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: max simulation rounds, caps overly long simulations
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it is stopped and run logs are cleared first
        - Cleared content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Config file (simulation_config.json) and profile files are NOT cleared
        - Useful for scenarios where you need to re-run the simulation
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // whether it was a force restart
            }
        }
    """
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is done, check if a process is currently running
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running, call /stop to stop it first, or use force=true to force restart"
                            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for real-time activity display on frontend.
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used to display progress bars and timeline views on frontend.
...
def get_agent_stats(simulation_id: str):
    ...
    Used to display Agent activity ranking, action distribution, etc. on frontend.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (waiting for commands after the simulation loop).
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has closed. Please ensure the simulation has completed and is in command-waiting mode."
            }), 400
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // if not specified: dual-platform mode interviews each Agent on both platforms
    ...
    try:
        ...
        # Optimize each interview prompt by adding prefix to prevent Agent from calling tools
