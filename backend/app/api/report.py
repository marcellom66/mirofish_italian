...
def _normalize_report_language(value: str) -> str:
    """Normalize frontend locale/language code to supported report language."""
    if not value:
        return 'en'
...
def generate_report():
    ...
    This is a long-running operation. The endpoint returns immediately with a task_id.
    ...
    try:
        ...
        report_language = _normalize_report_language(
            data.get('report_language') or data.get('language')
        )
        ...
        # Pre-generate report_id so it can be returned immediately to the frontend
        import uuid
...
def get_generate_status():
    """
    Get report generation task progress.
    ...
    try:
        ...
        # If simulation_id provided, check for an already-completed report first
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # MD file doesn't exist — generate a temporary file
            import tempfile
...
# ============== Report Progress and Sections Endpoints ==============
...
def get_report_sections(report_id: str):
    """
    Get the list of already-generated sections (streaming output).

    The frontend can poll this endpoint to receive completed sections without waiting for the full report.
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Interview is unlocked only once the report is completed
...
def get_agent_log(report_id: str):
    ...
    Returns real-time step-by-step actions during report generation, including:
    ...
    try:
        ...
        logger.error(f"failed to get Agent logs / impossibile ottenere i log Agent: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stream_agent_log(report_id: str):
    """
    Get complete Agent logs in one call / Ottieni i log Agent completi in una chiamata.
...
# ============== Console Log API / API Log Console ==============

@report_bp.route('/<report_id>/console-log', methods=['GET'])
def get_console_log(report_id: str):
    """
    Get Report Agent console output logs / Ottieni i log console del Report Agent.

    Fetches plain-text console-style output in real time (INFO, WARNING, etc.) /
    Recupera in tempo reale output testuale stile console (INFO, WARNING, ecc.).
    This differs from structured JSON returned by agent-log /
    Questo e diverso dal JSON strutturato restituito da agent-log.

    Query params / Parametri query:
        from_line: start reading from this line (optional, default 0, incremental fetch) /
            inizia a leggere da questa riga (opzionale, default 0, fetch incrementale)
    ...
    Returns / Restituisce:
        {
            ...
            "data": {
                "logs": [
                    "[19:46:14] INFO: search complete / ricerca completata: 15 related facts / fatti correlati",
                    "[19:46:14] INFO: graph search / ricerca grafo: graph_id=xxx, query=...",
                    ...
                ],
                "total_lines": 100,
                "from_line": 0,
    ...
    try:
        ...
        logger.error(f"failed to get console logs / impossibile ottenere i log console: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stream_console_log(report_id: str):
    """
    Get complete console logs in one call / Ottieni i log console completi in una chiamata.
...
# ============== Tool APIs (Debug) / API Strumenti (Debug) ==============

@report_bp.route('/tools/search', methods=['POST'])
def search_graph_tool():
    """
    Graph search tool API for debugging / API strumento ricerca grafo per debug.

    Request (JSON) / Richiesta (JSON):
        {
            "graph_id": "mirofish_xxxx",
            "query": "search query / query di ricerca",
            "limit": 10
        }
    """
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": "please provide graph_id and query / fornisci graph_id e query"
            }), 400
        ...
        logger.error(f"graph search failed / ricerca grafo fallita: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_graph_statistics_tool():
    """
    Graph statistics tool API for debugging / API statistiche grafo per debug.
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "please provide graph_id / fornisci graph_id"
            }), 400
        ...
        logger.error(f"failed to get graph statistics / impossibile ottenere statistiche grafo: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
