...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", possessing a "God's eye view" of the simulated world — you can observe every Agent's behavior, statements, and interactions within the simulation.
...
[Section Voice / Writing Persona]
{section_style_instruction}
...
DO focus on "what would the future look like" — simulation results ARE the predicted future
...
3. [Language Consistency - Quoted Content Must Match Report Language]
   ...
   - Ensure the report is written consistently in the same language throughout
   - When quoting content from tools, translate it into fluent language matching the report before including it
   - Maintain the original meaning while ensuring natural and smooth expression
...
5. [Hard Anti-Repetition Constraints]
    - Do not repeat the same claim in different wording within the same section
    - Do not reuse the same quote more than once in the same section
    - Avoid generic filler loops such as repeated "In summary" / "In conclusion" paragraphs
    - Each paragraph must add new information, a new angle, or a new stakeholder perspective
...
- Do NOT add a section heading at the beginning of the content
...
Weibo, as the first scene of public opinion, served as the core function for information first-release:
...
Strictly prohibited:
- Do not include both a tool call and Final Answer in a single response
...
7. [Emphasis Again] Do not add any headings! Use **bold text** instead of sub-section headings"""
...
5. Do not reuse claims/quotes that already appeared in previous sections unless absolutely necessary
6. Prefer concrete contrasts (who agrees vs who opposes, where, and why) over generic summaries
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "Please call more tools to obtain more simulation data before outputting Final Answer.{unused_hint}"
)
...
REACT_TOOL_LIMIT_MSG = (
    "Tool call count has reached the limit ({tool_calls_count}/{max_tool_calls}), no more tools can be called. "
...
REACT_FORCE_FINAL_MSG = "Tool call limit has been reached. Please directly output Final Answer: and generate the section content."
...
class ReportAgent:
    ...
    def __init__(
        ...
        logger.info(
            "ReportAgent initialization complete: "
            f"graph_id={graph_id}, simulation_id={simulation_id}, language={self.preferred_language}"
        )

    def _normalize_report_language(self, value: str) -> str:
        """Normalize locale/language values to supported language codes."""
        if not value:
            return "en"
    ...
    def _report_language_name(self) -> str:
        ...
        return self.SUPPORTED_REPORT_LANGUAGES.get(self.preferred_language, "English")

    def _report_language_instruction(self) -> str:
        """Return strict language instruction injected into prompts."""
        language_name = self._report_language_name()
        return (
            f"Write ALL output strictly in {language_name}. "
            "This includes title, summary, section titles, body text, and quoted content. "
            "Do not mix languages in the final output."
        )

    def _section_style_instruction(self, section_title: str, section_index: int) -> str:
        """Provide a distinct writing persona per section to reduce monotony."""
        title = (section_title or "").lower()

        if self.preferred_language == "it":
            if section_index == 1 or "stato" in title:
                return (
                    "Tono analitico-giornalistico: apri con un quadro chiaro e numerico, "
                    "poi collega i dati a 2-3 conseguenze concrete."
                )
            if section_index == 2 or "reazioni" in title:
                return (
                    "Tono sociologico-narrativo: evidenzia differenze tra gruppi sociali e territori, "
                    "con citazioni brevi e contrastanti."
                )
            if section_index == 3 or "tendenze" in title:
                return (
                    "Tono foresight/strategy: struttura per segnali deboli, rischi emergenti e opportunita, "
                    "evita frasi generiche ripetute."
                )
            if section_index == 4 or "conflitti" in title or "sfide" in title:
                return (
                    "Tono negoziale-conflittuale: mostra frizioni tra stakeholder con trade-off espliciti "
                    "(costi, tempi, equita, consenso)."
                )
            if section_index == 5 or "raccomand" in title:
                return (
                    "Tono policy-pragmatico: raccomandazioni operative, priorita, tempi e impatti attesi; "
                    "niente ripetizioni del contesto gia descritto."
                )
            return "Tono professionale ma concreto, con enfasi su specificita e zero ripetizioni."

        if section_index == 1 or "state" in title:
            return "Analytical-journalistic voice: lead with data-backed synthesis and concrete implications."
        if section_index == 2 or "reaction" in title:
            return "Socio-narrative voice: compare groups, motivations, and frictions with short contrasting quotes."
        if section_index == 3 or "trend" in title:
            return "Foresight strategist voice: focus on emerging signals, risk paths, and opportunity windows."
        if section_index == 4 or "conflict" in title or "challenge" in title:
            return "Negotiation/conflict voice: make trade-offs explicit across stakeholders."
        if section_index == 5 or "recommend" in title:
            return "Pragmatic policy-advisor voice: actionable recommendations with priorities and expected impact."
        return "Professional, specific, and non-repetitive voice."

    def _clean_section_content(self, content: str, previous_sections: List[str]) -> str:
        """Remove repetitive blocks and duplicate quotes from a generated section."""
        text = (content or "").strip()
        if not text:
            return text

        previous_fingerprints = set()
        for prev in previous_sections or []:
            for blk in re.split(r'\n\s*\n+', prev):
                norm = re.sub(r'\s+', ' ', blk).strip().lower()
                if norm:
                    previous_fingerprints.add(norm)
        ...
        for blk in blocks:
            ...
            # Remove exact repeated blocks in the same section.
            if norm in seen_norm:
                continue

            # Avoid repeating very similar starts (common loop symptom).
            prefix = norm[:120]
            if prefix in seen_prefix:
                continue

            # Drop blocks that are identical to previous sections.
            if norm in previous_fingerprints:
                continue

            if raw.startswith('>'):
                quote_norm = re.sub(r'\s+', ' ', raw.lstrip('> ').strip()).lower()
        ...
        # Keep at least one block to avoid empty section edge cases.
        if not kept:
            return text
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when Format 1 doesn't match, to avoid false matches with JSON in body text
    ...
    def plan_outline(
        ...
        system_prompt = (
            ...
            f"[Output Language]\n{self._report_language_instruction()}"
        )
        ...
        try:
            ...
            if self.preferred_language == "it":
                return ReportOutline(
                    ...
                    summary="Analisi di trend futuri e rischi basata sulle previsioni della simulazione",
                    sections=[
                        ReportSection(title="Scenario Predittivo e Risultati Chiave"),
                        ReportSection(title="Analisi Predittiva dei Comportamenti Collettivi"),
                        ReportSection(title="Prospettive dei Trend e Allerte di Rischio")
                    ]
                )
    ...
    def _generate_section_react(
        ...
        # Build user prompt - pass in max 4000 characters for each completed section
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break out of loop for forced finalization
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: degraded handling, truncate to first tool call, force execution
                    logger.warning(
            ...
            if has_final_answer:
                ...
                final_answer = self._clean_section_content(final_answer, previous_sections)
            ...
            # Directly use this content as the final answer, no more idle loops
            ...
            final_answer = self._clean_section_content(response.strip(), previous_sections)
        ...
        # Reached maximum iteration count, force content generation
    ...
    def generate_report(
        ...
        Each section is saved to folder immediately after generation, no need to wait for the entire report.
        File structure:
        reports/{report_id}/
        ...
        # Auto-generate if report_id not provided
        if not report_id:
...
class ReportManager:
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        logger.info(f"full report assembled / report completo assemblato: {report_id}")
        return md_content
    
    @classmethod
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        """
        Post-process report content / Post-processa contenuto report.

        1. Remove duplicate headings.
        2. Keep report main heading (#) and section headings (##), downgrade
           lower heading levels (###, ####, ...).
        3. Clean excessive empty lines and separators.

        Args:
            content: original report text / testo report originale
            outline: report outline / outline report

        Returns:
            post-processed content / contenuto post-processato
        """
        import re
        ...
        # Collect section titles from outline.
        section_titles = set()
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check duplicate heading within recent processed lines.
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate heading and trailing empty lines.
                    i += 1
                ...
                if level == 1:
                    if title == outline.title:
                        # Keep main report heading.
                        processed_lines.append(line)
                        prev_was_heading = True
                    elif title in section_titles:
                        # Section heading used '#': normalize to '##'.
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Non-section level-2 heading: convert to bold text.
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
    ...
    def save_report(cls, report: Report) -> None:
        """Save report metadata and full report / Salva metadati report e report completo."""
        ...
        logger.info(f"report saved / report salvato: {report.report_id}")
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatibility: check legacy flat file in reports dir.
    ...
    def get_report_by_simulation(cls, simulation_id: str) -> Optional[Report]:
        """Get report by simulation ID / Ottieni report per simulation ID."""
        cls._ensure_reports_dir()
        
        for item in os.listdir(cls.REPORTS_DIR):
            item_path = os.path.join(cls.REPORTS_DIR, item)
            # New format: directory-based report storage.
            if os.path.isdir(item_path):
                report = cls.get_report(item)
                if report and report.simulation_id == simulation_id:
                    return report
            # Backward compatibility: legacy JSON files.
    ...
    def delete_report(cls, report_id: str) -> bool:
        """Delete report (entire folder) / Elimina report (cartella completa)."""
        import shutil
        
        folder_path = cls._get_report_folder(report_id)
        
        # New format: remove entire report directory.
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            shutil.rmtree(folder_path)
            logger.info(f"report folder removed / cartella report rimossa: {report_id}")
            return True
        
        # Backward compatibility: delete legacy standalone files.
        deleted = False
