    ...
    def filter_defined_entities(
        ...
        for node in all_nodes:
            ...
            if not custom_labels:
                # Fallback: include generic or unlabeled nodes to avoid zero-entity deadlock.
                # Some graphs can contain nodes with labels=[] but meaningful name/summary.
                has_content = bool(node.get("name") or node.get("summary") or node.get("attributes"))
                is_generic_entity = ("Entity" in labels) or (not labels and has_content)
