...
# Flag to track whether the cleanup function has already been registered
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # max simulation rounds (optional, caps overly long simulations)
        enable_graph_memory_update: bool = False,  # whether to update Agent activity to Zep graph
        ...
        Args:
            ...
            max_rounds: Max simulation rounds (optional, caps overly long simulations)
        ...
        try:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # Full path to config file
            ]

            # Add max-rounds argument if specified
            if max_rounds is not None and max_rounds > 0:
            ...
            # This fixes issues with third-party libs (e.g. OASIS) that open files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+: makes all open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensures stdout/stderr use UTF-8

            # Set working directory to simulation dir (databases etc. will be created there)
            # Use start_new_session=True to create a new process group so os.killpg can terminate all children
            ...
            # Save file handle for later close
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        # Per-platform action log structure
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # Single-platform: check that platform only
                                    # Dual-platform: both must complete
                            ...
                            # Send activity to Zep if graph memory update is enabled
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # Determine which platforms are enabled (by file existence)
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = force kill, /T = terminate tree (including children)
            ...
            # Since start_new_session=True, process group ID equals the main process PID
            ...
            try:
                ...
                # Force SIGKILL if process group did not exit in time
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Platform: prefer value from record, fall back to default_platform
    ...
    def get_all_actions(
        ...
        """
        Get complete action history for all platforms (no pagination).
        ...
        # Read Twitter action file (auto-sets platform to "twitter")
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (auto-sets platform to "reddit")
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format should have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_timeline(
        ...
        """
        Get simulation timeline (summarized per round).

        Args:
            simulation_id: Simulation ID
            start_round: Start round (inclusive)
            end_round: End round (inclusive)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean (avoid printing useless log for empty processes)
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        # Process may already be gone — try direct terminate
                        try:
                            process.terminate()
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register in the reloader subprocess (the one that actually runs the app)
        ...
        # In debug mode: only register in the reloader subprocess; in non-debug: always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent the subprocess from re-trying
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Call original signal handler so Flask exits normally
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                if callable(original_sighup):
                    ...
                    # Default behaviour: normal exit
                    sys.exit(0)
            else:
                # Original handler not callable (e.g. SIG_DFL) — use default
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden per-item)
                - "twitter": Default to Twitter platform only
                - "reddit": Default to Reddit platform only
                - None: Interview both platforms per agent in dual-platform simulation
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        # Load all agent info from config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command so the simulation gracefully exits its command-waiting mode.
        ...
        try:
            ...
            # Timeout may mean the environment is already in the process of closing
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specific Agent ID (optional — returns only that agent's history)
        ...
        if platform in ("reddit", "twitter"):
            ...
            # When platform is not specified, query both
