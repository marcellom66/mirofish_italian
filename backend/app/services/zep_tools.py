...
1. InsightForge (Deep Insight Retrieval) - Most powerful hybrid retrieval, automatically generates sub-queries and retrieves across multiple dimensions
...
class InsightForgeResult:
    ...
    Contains retrieval results from multiple sub-queries, along with integrated analysis
    """
    query: str
...
class ZepToolsService:
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (with pagination, including temporal information)
...
Return the sub-question list in JSON format."""
    ...
    def panorama_search(
        ...
        Gets a full panoramic view, including all relevant content and historical/expired information:
        1. Gets all relevant nodes
        2. Gets all edges (including expired/invalidated ones)
        3. Categorizes and organizes currently valid and historical information
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                meaningful = [
                    ...
                    and not re.match(r'^[\s\W,;:\s]+', s.strip())
                    and not s.strip().startswith(('{', 'Question'))
                ]
                meaningful.sort(key=len, reverse=True)
                key_quotes = [s + "." for s in meaningful[:3]]

                # Strategy 2 (supplementary): Properly paired quotation marks with long text
                if not key_quotes:
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: List of full information for selected Agents
...
Please select up to {max_agents} Agents most suitable for interviewing, and explain your selection reasoning."""
