...
Uses LLM to auto-generate detailed simulation parameters based on simulation requirements,
document content, and graph information. Fully automated, no manual parameter setting needed.
...
# Daily schedule time configuration (Beijing time)
...
class TimeSimulationConfig:
    """Time simulation configuration (based on daily schedule habits)"""
    ...
    # Agent activation count range per hour
...
class SimulationConfigGenerator:
    ...
    to auto-generate optimal simulation parameter configurations.
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # 500 char margin
...
- The user population follows local daily schedule habits
- Late night 0-5 AM: almost no activity (activity multiplier 0.05)
- Morning 6-8 AM: gradually active (activity multiplier 0.4)
- Work hours 9 AM-6 PM: medium activity (activity multiplier 0.7)
...
- **Important**: The example values below are for reference only. Adjust specific time periods based on event nature and participant characteristics
  - Example: Student groups may peak at 9-11 PM; media active all day; official institutions only during work hours
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should follow realistic daily schedule patterns."
...
- **Schedule follows realistic patterns**: Late night 0-5 AM almost no activity, evening 7-10 PM most active
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, considering daily schedule>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should follow realistic daily schedule patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based generation of a single Agent configuration"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Expert/Professor: active during work + evening hours, medium frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
